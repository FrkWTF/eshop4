class CheckoutController < ApplicationController
  ...
  def submit_order
    ...
    if @order.save
      if @order.process
        flash[:notice] = 'Su pedido ha sido confirmado y sera procesado inmediatamente.'
        session[:order_id] = @order.id
        @cart.cart_items.destroy_all # empty shopping cart
        redirect_to :action => 'thank_you'
      else
        flash[:notice] = "Error confirmando pedido '#{@order.error_message}'."
        render :action => 'index'
      end
    else
      render :action => 'index'
  ...
  def populate_order
    for cart_item in @cart.cart_items
      order_item = OrderItem.new(:liquor_id => cart_item.liquor_id,
